...
def test_index_uses_single_import_entry() -> None:
    ...
    assert "导入与校对" in html
    assert "Excel 和图片请分开导入" in html
    assert "导入 Excel" in html
    assert "导入图片" in html
...
def test_legacy_excel_import_route_still_dispatches() -> None:
    with _tmp_workdir():
        workbook = Workbook()
        sheet = workbook.active
        sheet.title = "七上期中"
...
def test_legacy_image_import_route_still_dispatches() -> None:
    with _tmp_workdir():
        image = Image.new("RGB", (40, 40), "white")
        payload = BytesIO()
        image.save(payload, format="PNG")
...
def test_unified_import_accepts_image_mimetype_without_known_suffix() -> None:
    with _tmp_workdir():
        payload = BytesIO(b"not a real image but OCR is mocked")

        with patch("student_analysis.web.image_to_text", return_value=("张三 88 1", None)) as ocr:
            response = create_app().test_client().post(
                "/import/files",
                data={"file": (payload, "clipboard", "image/png")},
                content_type="multipart/form-data",
            )
...
if __name__ == "__main__":
    ...
    test_legacy_excel_import_route_still_dispatches()
    test_unified_import_dispatches_image_file()
    test_legacy_image_import_route_still_dispatches()
    test_unified_import_accepts_image_mimetype_without_known_suffix()

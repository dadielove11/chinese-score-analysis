...
三、导入成绩
1. 优先使用 Excel 导入：点击“Excel 导入”，可一次选择一个或多个 .xlsx/.xlsm 文件。
2. Excel 上传后会进入确认页，按文件和工作表显示识别情况；如果没有读出班级或考试名称，可在确认页逐项手动填写。
3. 图片导入适合表格截图：点击“图片导入”，可一次选择一张或多张图片。
4. 图片上传后每张图会单独显示一个校对区域，请逐张确认班级、考试名称和识别结果，再统一导入。
